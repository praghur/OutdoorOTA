...
tmux split-window -v
sudo tcpdump -i enx76da8884639d -w ue1_capture.pcap
tmux split-window -v
...
sudo apt install -y tshark
#Start packet capture
sudo tshark -i enxde923debed41 -T fields -e frame.time_epoch -e ip.src -e ip.dst -e ip.id -e udp.srcport -e udp.dstport -E header=y -E separator=, -E quote=d > ueTraff_results.csv
tmux split-window -v
sudo tcpdump -i enxde923debed41 -w ueTraff_capture.pcap
tmux split-window -v
/local/repository/bin/module-off.sh
/local/repository/bin/module-flight.sh
/local/repository/bin/module-on.sh
...
sudo apt install -y tshark
#Start packet capture
sudo tshark -i enxee981cd1dd36 -T fields -e frame.time_epoch -e ip.src -e ip.dst -e ip.id -e udp.srcport -e udp.dstport -E header=y -E separator=, -E quote=d > ue2_results.csv
tmux split-window -v
sudo tcpdump -i enxee981cd1dd36 -w ue2_capture.pcap
tmux split-window -v
/local/repository/bin/module-off.sh
/local/repository/bin/module-flight.sh
/local/repository/bin/module-on.sh
